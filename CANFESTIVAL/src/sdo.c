...
UNS8 sendSDO (CO_Data* d, UNS8 whoami, UNS8 CliServNbr, UNS8 *pData)
{
	...
	return canSend(d->canHandle,&m);//发送SDO
}
...
UNS8 proceedSDO (CO_Data* d, Message *m)
{
	...
	/* Am-I a server ? */
	offset = d->firstIndex->SDO_SVR;  //5
	lastIndex = d->lastIndex->SDO_SVR;//5
	j = 0;
	...
	if (whoami == SDO_UNKNOWN)   //从站不应进来 客户端是主站
	{
		/* Am-I client ? */
		offset = d->firstIndex->SDO_CLT;
	...
	if (whoami == SDO_UNKNOWN) //不是从站要的SDO
	{
	...
	err = getSDOlineOnUse( d, CliServNbr, whoami, &line );//查询已准备的SDO 0：找到
	...
	/* Allowed : cs = 0, 1, 2, 3, 4, 5, 6 *///1：写命令SDO
    ...
    //接收到一个写命令SDO
		case 1:
			...
			if (whoami == SDO_SERVER) {
				...
				/*Sending a SDO, cs=3*/
				data[0] = 3 << 5;                      //0x60 写成功应答
				data[1] = (UNS8)(index & 0xFF);        /* LSB */
				data[2] = (UNS8)((index >> 8) & 0xFF); /* MSB */
				data[3] = subIndex;
				for (i = 4 ; i < 8 ; i++)
					data[i] = 0;
				sendSDO(d, whoami, CliServNbr, data);//向主机 客户端发送写成功应答
			} /* end if I am SERVER */
			else {
				/* I am CLIENT *///主站
